...
from todo_app.data.session_items import  markcomplete,putToDoItems,getallitems,ViewModel,getonlytodoitems,getallitems_selenium
...
def create_app1():
    ...
    @app.route('/', methods=['GET'])
    def index():
        listofitems = getallitems_selenium() ## Get a list of all items (to do and done)
        #listofitems = getonlytodoitems() ##Get a list of all items (to do only)
        sortedListofitems = sorted(listofitems, key=lambda item: item.status,reverse=True )
